  ...
  <div class="container">
    ...
    <div class="main_box ">
      <div >
        <div class="content_style">
          <div>
            <h1 class="movie_title"> {{ movieData.title  }}</h1>
            <div style="display: flex;">
              <div class="btn_style">{{ movieData.runtime }}분</div>
              <div class="btn_style">{{ movieData.prodYear }}</div>
              <div class="btn_style">{{ movieNation }}</div>

              <div v-if="movieData.rating ==='전체관람가'" class="all_age_type">{{ movieData.rating }}</div>
              <div v-if="movieData.rating ==='18세관람가(청소년관람불가)'" class="eightteen_age_type">{{ movieData.rating }}</div>
              <div v-if="movieData.rating ==='15세관람가'" class="fifteen_age_type">{{ movieData.rating }}</div>
              <div v-if="movieData.rating ==='12세관람가'" class="twelve_age_type">{{ movieData.rating }}</div>
                <a class="teaser" id="open-calc-modal" @click="isTeaserViewed=true">예고편 보러가기
                <img   src="@/assets/icon/video3.png" alt="" style="height: 20px;"></a>
              <div  id="calc-modal">
                <div class="calc-modal-content">
                  <div  style="display: flex; justify-content:space-between;" > 
                    <span  style="margin:0px; margin-bottom: 10px; color:white; font-size: 30px;">[ 예고편 ] {{ movieData.title }}</span>
                    <span  id="close-calc-modal" style="font-size: 30px; color:white; cursor: pointer; ">x</span>
                  </div>
                    <div  style="text-align: center; ">  
                              <iframe v-if="movieData.vods" :src="`${movieData.vods}`" style="border-radius: 5px; border: hidden;" width="900px" height="600px"></iframe>
                              <p v-else class="content_text">해당 예고편이 없습니다.</p>
                    </div>
                </div>
              </div>
            </div> 
            <p class="content_text">크리에이터  &nbsp; {{ moviedirector }}</p>
            <p class="content_text">개봉일  &nbsp; {{ movieData.repRlsDate }}</p>
            <p class="content_text">{{ movieData.plots }}</p>
            <div style="display: grid; grid-template-columns: 5fr 1fr;">
              <div style="width: 800px;">      
                <p class="content_text">출연  &nbsp;  
                  <span v-for="actor in movieData.actors">
                    {{ actor }}, 
                  </span></p> 
                <p class="content_text"> 
                  <template v-for="genre in movieData.genre">
                    <span v-if="genre !== 'no'"
          ...
          <div>
            <img class="img_style" :src="moviePoster" alt="" > 
          </div>
        </div>
      </div> 
    </div>
    <div style="border-bottom: white 1px solid ; padding-left: 30px; padding-right: 30px;">
      ...
      <Carousel style="margin:0px" :autoplay="2000" :itemsToShow="5" :wrapAround="true" :transition="500">
        <Slide v-for="still in movieImage.stlls " :key="still">
    ...
    <div style="padding-left: 45px;">
      ...
      <div style="margin-bottom: 20px;">
        <form @submit.prevent="create">
          ...
          <input class="comment_btn" type="submit" value="댓글 입력">
        </form>
      </div>
    </div>
  </div> 
...
onMounted(() =>{
  ...
  .then ((res) =>{
    ...
    moviedirector.value = movieData.value.directors[0]
    movieNation.value = movieData.value.nation[0]
  })  
  .catch((error) => {
    console.log(error)
  })
  axios({
    method:'get',
    url: `${movieStore.API_URL}/api/v1/movies/${route.params.id}/image/`
  })
  .then ((res) =>{
    movieImage.value = res.data
    moviePoster.value = movieImage.value.poster[0]
  })  
  /* 모달창 */
  const calcModal = document.getElementById("calc-modal");
    ...
    axios({
    method: 'GET',
    url: `${userStore.API_URL}/api/v1/movies/${route.params.id}/`
  })
    .then((res) => {
...
const create = function () {
  axios({
    method: 'POST',
    url: `${userStore.API_URL}/api/v1/movies/comment/${route.params.id}/`,
    data: {
...
const likeEvent = function () {
  axios({
    method: 'POST',
    url: `${userStore.API_URL}/api/v1/movies/like/${route.params.id}/`,
    headers: {
      Authorization : `Token ${userStore.token}`
    }
  })
