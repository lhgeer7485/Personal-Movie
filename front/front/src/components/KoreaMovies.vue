    ...
    <p class="title title_font">
      ...
      <div>
        ...
        <Carousel style="margin:0px"  :autoplay="2000"  :itemsToShow="5" :wrapAround="true" :transition="500">
          <Slide v-for="(kmv, index) in  koreaMovie" :key="index">
            <div class="carousel__item "> 
              ...
              @mouseenter="mouse1"
              @mouseleave="mouse2"
              @click="goKoreamovie(kmv.id, koreaMovie, newMovies[kmv.id])"
              :koreaMovie="koreaMovie"
              >
...
const koreaMovie = ref([])
const newMovies = ref([])
onMounted(() =>{
  axios({
    method:'get',
    url: `${store.API_URL}/api/v1/movies/main/korea/`
  })
  .then((res) =>{
    // console.log(res)
    koreaMovie.value = res.data
    // console.log(koreaMovie)
    let arr = []
    let temp = ''
    for(let i = 0; i<koreaMovie.value.length; i++){
      temp = koreaMovie.value[i].posters.split("'")[1]
      arr.push(temp)
    }
    newMovies.value = arr
...
const goKoreamovie = function(index, item, poster){
